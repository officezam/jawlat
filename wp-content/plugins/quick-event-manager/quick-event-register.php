			...
			if (!$verify) {
				...
				*/
				...
				if (empty($formerrors)) {
					...
					if (($ic['useincontext'] == 'checked') && $paypal && $cost && !$formvalues['ignore']) {
                    ...
                    if ($num == 0 && $event['active_buttons']['field11'] && $number && $register['waitinglist']) {
...
function qem_loop() {
	...
	$uic = $ic['useincontext'] || '';
	if ($ic['useincontext'] && (isset($_GET['token']) && isset($_GET['PayerID']))) {
	...
	} elseif ($ic['useincontext'] == 'checked' && isset($_GET['token'])) {
...
function qem_display_form( $values, $errors, $registered ) {
    ...
    if ($errors['spam']) {
        ...
        $content .= '<div class="qem-register"><div id="' . $style['border'] . '">';
        if (count($errors) > 0) {
...
function qem_process_form($values, $ajax = false) {
    ...
    . "Content-Type: text/html; charset=\"utf-8\"\r\n";	
        ...
        if ($register['qemmail'] == 'smtp') {
            ...
            qem_send_smtp($add_email, $notificationsubject, $values, $message);
        } else {
            wp_mail($qem_email, $notificationsubject, $message, $headers);
            wp_mail($add_email, $notificationsubject, $message, $headers);
        }
    }
    ...
    if (isset($addons['mailchimpuser']) && $addons['mailchimpuser'] && $values['email'])
        qem_mailchimp($values,$addons);
