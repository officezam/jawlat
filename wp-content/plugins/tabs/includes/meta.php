...
function meta_boxes_tabs_input( $post ) {
	...
	$from_price     	 = get_post_meta( $post->ID, 'from_price', true );
	$to_price       	 = get_post_meta( $post->ID, 'to_price', true );
	$tabs_search_heading = get_post_meta( $post->ID, 'tabs_search_heading', true );


	$destination    = get_post_meta( $post->ID, 'destination', true );
	$tab_language   = get_post_meta( $post->ID, 'tab_language', true );
    ...
    <div class="para-settings">
        ...
        <ul class="tab-nav"> 
	        ...
	        <li nav="6" class="nav6">Custom Search</li>
		...
		<ul class="box">
            ...
            <li style="display: none;" class="box3 tab-box">
				<div class="option-box">
                    ...
                    <li theme="flat" class="<?php if($tabs_themes=="flat")echo "selected"; ?>" >
	                    <img src="<?php echo $themes_dir_path.'flat.png'; ?>" />
                    </li>
                    <li theme="rounded-top" class="<?php if($tabs_themes=="rounded-top")echo "selected"; ?>">
	                    <img src="<?php echo $themes_dir_path.'rounded-top.png'; ?>" />
                    </li>
            ...
            <li style="display: none;" class="box4 tab-box">
				...
				<div class="option-box">
                    ...
                    <div id="tabs-content" class="tabs-content expandable">
						...
						if(empty($tabs_search_heading))
						{
							$tabs_search_heading = array('0'=>'');
						}
						if(empty($duration))
						{
							$duration = array('0'=>'');
                            ...
                            <div class="items">
                                ...
                                <input width="100%" placeholder="Tabs Header" type="text" name="tabs_content_title[<?php echo $index; ?>]" value="<?php if(!empty($tabs_title)) echo $tabs_title; ?>" />
									...
									<table id="search">
										<tbody>
										<tr>
											<td><strong>Select Package Type: </strong></td>
											<td>
												<select name="package[<?php echo $index; ?>]">
													<option value="honeymoon" <?php if($package[$index] == 'honeymoon' ) echo 'selected'; ?> >Honey Moon Package </option>
													<option value="holiday" <?php if($package[$index] == 'holiday' ) echo 'selected'; ?> >Holiday Package </option>
													<option value="family" <?php if($package[$index] == 'family' ) echo 'selected'; ?> >Family Package </option>
												</select>
											</td>
										</tr>
										<tr>
											<td><strong>Maximum Duration Days: </strong></td>
											<td>

												<select name="duration[<?php echo $index; ?>]">
													<option value="3" <?php if($duration[$index] == '3' ) echo 'selected'; ?> >1 - 3 Days</option>
													<option value="6" <?php if($duration[$index] == '6' ) echo 'selected'; ?> >4 - 6 Days</option>
													<option value="9" <?php if($duration[$index] == '9' ) echo 'selected'; ?> >7 - 9 Days </option>
												</select>
...
<!--											<td><input type="number" name="people[--><?php //echo $index; ?><!--]" value="--><?php //if(!empty($people)) echo $people[$index]; ?><!--" ></td>-->
<!--										</tr>-->
										<tr>
											<td><strong>Date From: </strong></td>
											<td><input type="date" name="from_date[<?php echo $index; ?>]" value="<?php if(!empty($from_date)) echo $from_date[$index]; ?>" ></td>
										</tr>

										<tr>
											<td><strong>Date To: </strong></td>
											<td><input type="date" name="to_date[<?php echo $index; ?>]" value="<?php if(!empty($to_date)) echo $to_date[$index]; ?>" ></td>
										</tr>

										<tr>
											<td><strong>Price From: </strong></td>
											<td><input type="text" name="from_price[<?php echo $index; ?>]" value="<?php if(!empty($from_price)) echo $from_price[$index]; ?>" ></td>
										</tr>

										<tr>
											<td><strong>Price To: </strong></td>
											<td><input type="text" name="to_price[<?php echo $index; ?>]" value="<?php if(!empty($to_price)) echo $to_price[$index]; ?>" ></td>
										</tr>
										<tr>

											<td colspan="2">
												<label><strong>Short Description</strong></label>
													<textarea rows="4" name="tabs_search_heading[<?php echo $index; ?>]"><?php if(!empty($tabs_search_heading)) echo $tabs_search_heading[$index]; ?></textarea>
											</td>
										</tr>
										</tbody>
									</table>
...
?>
			...
			<div  class="option-box">
				...
				<div id="location_box" class="postbox ">
					<button type="button" class="handlediv button-link" aria-expanded="true">
						<span class="screen-reader-text">Toggle panel: Add Search Detail</span>
						<span class="toggle-indicator" aria-hidden="true"></span>
					</button>
					<h2 class="hndle ui-sortable-handle">
						<span>Add Search Detail</span>
					</h2>
					...
					<div class="inside">
						<table id="search">
							...
							<tr style="height:38px;">
								...
								<td><input type="text" name="destination" value="<?php if(!empty($destination)) echo $destination; ?>"  id="name1"></td>
							</tr>

							<tr>
								<td><strong>Tabs Language: </strong></td>
								<td>
									<strong>English </strong><input type="radio" name="tab_language" value="english" <?php if( $tab_language == 'english'){ ?> checked <?php } ?> >
									<strong>Arabic </strong><input type="radio" name="tab_language" value="arabic" <?php if( $tab_language == 'arabic'){ ?> checked <?php } ?> ></td>
							</tr>

							<tr >
								<td><strong>Post Url: </strong></td>
								<td><input type="text" name="page_url" value="<?php if(!empty($page_url)) echo $page_url; ?>"></td>
							</tr>
			...
			<!-- End of AMIR Add Code For Search  -->
	...
	<script>
		jQuery(document).ready(function(jQuery)
		{
			jQuery(".tabs_language").change(function()
			{
				if( this.value == 'arabic'){
					alert(this.value);
					//$("#name1").attr("dir", "rtl");
					$("#name1").addClass("right-to-left");//css("direction","rtl");

				}else{
					alert(this.value);
					$("#name1").addClass("right-to-left");
				}
			})
		})
...
function meta_boxes_tabs_save( $post_id ) {
  ...
  // Sanitize user input.
	...
	$package 			 = stripslashes_deep( $_POST['package'] );
	$duration 			 = stripslashes_deep( $_POST['duration'] );
	$people 			 = stripslashes_deep( $_POST['people'] );
	$from_date 			 = stripslashes_deep( $_POST['from_date'] );
	$to_date 			 = stripslashes_deep( $_POST['to_date'] );
	$from_price 		 = stripslashes_deep( $_POST['from_price'] );
	$to_price 			 = stripslashes_deep( $_POST['to_price'] );
	$tabs_search_heading = stripslashes_deep( $_POST['tabs_search_heading'] );
	...
	//Amir Custom Search Fields Save
	$destination    = sanitize_text_field( $_POST['destination'] );
	$tab_language   = sanitize_text_field( $_POST['tab_language'] );
  ...
  // Update the meta field in the database.
	...
	update_post_meta( $post_id, 'from_price'    ,   $from_price );
	update_post_meta( $post_id, 'to_price'      ,   $to_price );
	update_post_meta( $post_id, 'tabs_search_heading', $tabs_search_heading );
	...
	update_post_meta( $post_id, 'destination'   ,   $destination );
	update_post_meta( $post_id, 'tab_language'  ,   $tab_language );
